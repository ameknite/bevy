...
pub enum FogFalloff {
    ...
    /// <text font-family="sans-serif" fill="#FF00E5" style="white-space: pre" font-family="Inter"
    /// font-size="10" letter-spacing="0em"><tspan x="267" y="58.6364">end</tspan></text>
    /// </svg>
    Linear {
    ...
    ///   the scene
    /// scale. Typically, for scenes with objects in the scale of thousands of units, you might
    /// want density values in the ballpark of `0.001`. Conversely, for really small scale
    /// scenes you might want really high values of density;
    /// - Combine the `density` parameter with the [`FogSettings`] `color`'s alpha channel for
    ...
    ///
    /// <svg width="370" height="212" viewBox="0 0 370 212" fill="none">
    /// <title>Plot showing how exponential fog falloff behaves for different density
    /// values</title> <mask id="mask0_3_31" style="mask-type:alpha" maskUnits="userSpaceOnUse"
    ...
    /// fill="currentColor" style="white-space: pre" font-size="12" letter-spacing="0em"><tspan
    /// x="0" y="11.8636">fog intensity</tspan></text> </svg>
    Exponential {
    ...
    /// 53.1724" stroke="#001AFF" stroke-width="1"/> <path d="M42 150C166.394 150 164.42
    /// 53.1724 504.035 53.1724L1388 53.1724" stroke="#14FF00" stroke-width="1"/> </g>
    /// <path d="M331 151H42V49" stroke="currentColor" stroke-width="2"/>
    /// <text font-family="sans-serif" fill="currentColor" style="white-space: pre" font-size="12"
    /// letter-spacing="0em"><tspan x="136" y="173.864">1</tspan></text> <text font-family="
    /// sans-serif" fill="currentColor" style="white-space: pre" font-size="12"
    ...
    /// atmospheric scattering model. Additionally, individual color channels can have their
    /// own density values, resulting in a total of six different configuration parameters.
    ///
    /// ## Tips
    ///
    /// - Use the [`FogFalloff::from_visibility_colors()`] or
    ///   [`FogFalloff::from_visibility_color()`] convenience methods
    /// to create an atmospheric falloff with the proper densities for a desired visibility
    /// distance in world units and extinction and inscattering colors;
    /// - Combine the atmospheric fog parameters with the [`FogSettings`] `color`'s alpha channel
