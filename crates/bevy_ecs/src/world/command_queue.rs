...
impl CommandQueue {
    ...
    fn apply_or_drop_queued(&mut self, mut world: Option<&mut World>) {
        ...
        // Create a stack for the command queue's we will be applying as commands may queue
        // additional commands. This is preferred over recursion to avoid stack overflows.
        ...
        while let Some((mut cursor, mut end)) = resolving_commands.pop() {
            while cursor < end {
                // SAFETY: The cursor is either at the start of the buffer, or just after the
                // previous command. Since we know that the cursor is in bounds, it
                // must point to the start of a new command.
                ...
                // SAFETY: For most types of `Command`, the pointer immediately following the
                // metadata is guaranteed to be in bounds. If the command is a
                // zero-sized type (ZST), then the cursor might be 1 byte past the
                // end of the buffer, which is safe.
                cursor = unsafe { cursor.add(std::mem::size_of::<CommandMeta>()) };
                // Construct an owned pointer to the command.
                // SAFETY: It is safe to transfer ownership out of `self.bytes`, since the call to
                // `set_len(0)` above guarantees that nothing stored in the buffer
                // will get observed after this function ends. `cmd` points to a
                // valid address of a stored command, so it must be non-null.
                let cmd = unsafe {
                ...
                // or 1 byte past the end, so this addition will not overflow the pointer's
                // allocation.
                cursor = unsafe { cursor.add(size) };

                if let Some(world) = &mut world {
                    // If the command we just applied generated more commands we must apply those
                    // first
                    if !world.command_queue.is_empty() {
                        // If our current list of commands isn't complete push it to the
                        // `resolving_commands` stack to be applied after
                        if cursor < end {
