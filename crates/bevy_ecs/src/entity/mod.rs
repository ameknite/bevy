...
impl Entities {
    ...
    pub fn reserve_entities(&self, count: u32) -> ReserveEntitiesIterator {
        ...
        let range_end = self
            .free_cursor
            // Unwrap: these conversions can only fail on platforms that don't support 64-bit
            // atomics and use AtomicIsize instead (see note on `IdCursor`).
    ...
    pub unsafe fn flush_and_reserve_invalid_assuming_no_entities(&mut self, count: usize) {
        ...
        *free_cursor = 0;
        ...
        // SAFETY: The EntityMeta struct only contains integers, and it is valid to have all bytes
        // set to u8::MAX
        unsafe {
            self.meta.as_mut_ptr().write_bytes(u8::MAX, count);
        }
        // SAFETY: We have reserved `count` elements above and we have initialized values from index
        // 0 to `count`.
        unsafe {
