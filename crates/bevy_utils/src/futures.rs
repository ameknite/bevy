...
fn noop_waker() -> Waker {
    // SAFETY: the `RawWakerVTable` is just a big noop and doesn't violate any of the rules in
    // `RawWakerVTable`s documentation (which talks about retaining and releasing any
    // "resources", of which there are none in this case)
