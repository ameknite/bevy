...
impl Camera {
    ...
    ///   [`logical_viewport_size`](Camera::logical_viewport_size)
    /// - The world coordinates cannot be mapped to the Normalized Device Coordinates. See
    ///   [`world_to_ndc`](Camera::world_to_ndc)
    ...
    /// - The logical viewport size cannot be computed. See
    ///   [`logical_viewport_size`](Camera::logical_viewport_size)
    /// - The near or far plane cannot be computed. This can happen if the `camera_transform`, the
    ///   `world_position`, or the projection matrix defined by [`CameraProjection`] contain `NAN`.
    ...
    ///   [`ndc_to_world`](Camera::ndc_to_world)
    ...
    /// defined by [`CameraProjection`] contain `NAN`. Panics if the `camera_transform` contains
    /// `NAN` and the `glam_assert` feature is enabled.
    pub fn world_to_ndc(
        &self,
        camera_transform: &GlobalTransform,
        world_position: Vec3,
    ...
    ///
    /// Returns `None` if the `camera_transform`, the `world_position`, or the projection matrix
    /// defined by [`CameraProjection`] contain `NAN`. Panics if the projection matrix is null
    /// and `glam_assert` is enabled.
...
pub enum CameraOutputMode {
    ...
    Write {
        ...
        /// The color attachment load operation that will be used by the pipeline that writes the
        /// intermediate render textures to the final render target texture.
    ...
    /// write to the render target using [`CameraOutputMode::Write`]. The "skip" mode can
    /// easily prevent render results from being displayed, or cause them to be lost. Only use
    /// this if you know what you are doing! In camera setups with multiple active cameras
    /// rendering to the same RenderTarget, the Skip mode can be used to remove unnecessary /
    /// redundant writes to the final output texture, removing unnecessary render passes.
    Skip,
}
...
/// projection if needed. It also queries any [`CameraProjection`] component associated with the
/// same entity as the [`Camera`] one, to automatically update the camera projection matrix.
///
