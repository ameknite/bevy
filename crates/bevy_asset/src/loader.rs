...
/// asset load state. Any asset state accessed by [`LoadContext`] will be tracked and stored for use
/// in dependency events and asset preprocessing.
...
impl<'a> LoadContext<'a> {
    ...
    /// "Finishes" this context by populating the final [`Asset`] value (and the erased
    /// [`AssetMeta`] value, if it exists). The relevant asset metadata collected in this
    /// context will be stored in the returned [`LoadedAsset`].
    ...
    /// Loads the [`Asset`] of type `A` at the given `path` with the given [`AssetLoader::Settings`]
    /// settings `S`. This is a "deferred" load. If the settings type `S` does not match the
    /// settings expected by `A`'s asset loader, an error will be printed to the log
    ...
    /// Loads the asset at the given `path` directly from the provided `reader`. This is an async
    /// function that will wait until the asset is fully loaded before returning. Use this if
    /// you need the _value_ of another asset in order to load the current asset, and that value
    /// comes from your [`Reader`]. For example, if you are deriving a new asset from the
    /// referenced asset, or you are building a collection of assets. This will add the `path` as a
    /// "load dependency".
    ///
    /// If the current loader is used in a [`Process`] "asset preprocessor", such as a
    /// [`LoadTransformAndSave`] preprocessor, changing a "load dependency" will result in
