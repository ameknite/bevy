...
//! 2. Use a [`Local`] [`ManualEventReader`] instead of an [`EventReader`], and use [`ResMut`] to
//!    access [`Events`].
//!
...
fn send_and_receive_manual_event_reader(
    ...
    // `ManualEventReader<T>` is the internal state of `EventReader<T>`, which tracks which events
    // have been seen.
