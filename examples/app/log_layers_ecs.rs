...
//!
//! Inside the [`update_subscriber`] function we will create a [`mpsc::Sender`] and a
//! [`mpsc::Receiver`] from a [`mpsc::channel`]. The [`Sender`](mpsc::Sender) will go into the
//! [`AdvancedLayer`] and the [`Receiver`](mpsc::Receiver) will go into a non-send resource called
//! [`LogEvents`] (It has to be non-send because [`Receiver`](mpsc::Receiver) is [`!Sync`](Sync)).
//! From there we will use [`transfer_log_events`] to transfer log events from [`LogEvents`] to an
//! ECS event called [`LogEvent`].
//!
